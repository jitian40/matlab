%总体方向图函数
function F=f(theta,phi,pop,I_phi,et)%参数包含球坐标下theta角与phi角，激励相位的初始位置，激励相位数组
        ...
        for Ai=1:length(phi_Z)%确定层数
         zarc=phi_Z(Ai);%柱坐标下phi角
            for Zi=1:length(d_busbar)%母线上单元方向图的叠加
                ...
                e=e+phi_i*ft.*exp(1i*(2*pi/15.*(sind(theta).*cosd(phi-zarc).*(R-d_z/2)+d_z.*cosd(theta))));%方向图函数的复数域的叠加(matlab可以直接计算复数)s
