...
function dirGrah=xchange(zarc,yarc,t,p,phi_i,zn,R)%单元天线的排列角度，参数包含圆锥体的底角，球坐标下theta角与phi角，激励相位，单元的Z坐标值，椎体的底面圆的半径
    ...
    dirGrah=phi_i.*sin(5/2.*pi.*cos(theta_2))./(5*sin(pi/2.*cos(theta_2))).*exp(1i*(-2*pi/15.*(sin(t).*cos(p-zarc).*(R-zn/2)+zn.*cos(t))));
end
...
function NewPop=Roulette(fit,pop)
    nans=isnan(fit);
    fitvalue=fit./sum(fit);%有一定几率出现nan值
