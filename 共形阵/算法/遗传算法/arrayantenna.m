...
function F=f(theta,phi,pop_num,I_phi)%参数包含球坐标下theta角与phi角，激励相位的初始位置，激励相位数组
    phi_Z=[16.77*pi/180,65.59*pi/180,114.41*pi/180,163.22*pi/180];%单元分布角度
    d_busbar=[0];
    R=9.6;%地面圆半径
    Phi_Y=pi/2;%椎体底角
    e=zeros(100,100);
...
function dirGrah=xchange(zarc,yarc,t,p,phi_i,zn,R)
    ...
    dirGrah=sin(5/2.*pi.*cos(theta_2))./(5*sin(pi/2.*cos(theta_2))).*exp(1j*(2*pi/10.*(sin(t).*cos(p-zarc).*(R-zn/2)+zn.*cos(t))));
end
